...
export default function SocialMediaPage() {
  ...
  async function movePipeline(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to move content.");
      }
                    ...
                    items.map((item) => (
                      ...
                      >
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-3">
                          {PIPELINE.indexOf(item.status) > 0 && (
                            <button
                              type="button"
                              onClick={() =>
                                void movePipeline(item, -1)
                              }
                              className="rounded-lg border border-[#D7D0C5] bg-white px-3 py-1.5 text-xs font-semibold text-[#6F776B]"
                            >
                              ←{" "}
                              {statusLabel(
                                PIPELINE[
                                  PIPELINE.indexOf(item.status) - 1
                                ],
                              )}
                            </button>
                          )}

                          {PIPELINE.indexOf(item.status) <
                            PIPELINE.length - 1 && (
                            <button
                              type="button"
                              onClick={() =>
                              ...
                              className="rounded-lg bg-[#1E3A34] px-3 py-1.5 text-xs font-semibold text-white"
                            >
                              {statusLabel(
                                PIPELINE[
                                  PIPELINE.indexOf(item.status) + 1
